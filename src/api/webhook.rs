...
pub async fn handle_appid_webhook(
    ...
    let is_handshake = serde_json::from_str::<serde_json::Value>(&body)
        .ok()
        .is_some_and(|data| {
            data.get("d")
                ...
                && data
                    .get("d")
                    .and_then(|value| value.get("plain_token"))
                    .and_then(|value| value.as_str())
                    .is_some()
        });
    ...
    if !is_handshake {
        ...
        debug!(
            "[Webhook验签] appid={}, timestamp={}, nonce={}",
            appid, ts, nonce
        );
        if !helpers::verify_signature(&secret, sig, ts, nonce, &body) {
            tracing::warn!("[Webhook验签失败] appid={}", appid);
            return (
                StatusCode::UNAUTHORIZED,
...
async fn process_webhook(
    ...
    let (raw_enabled, raw_path) = {
        ...
        (config.raw_content.enabled, config.raw_content.path.clone())
    };
    ...
    if let Some(ref fwd_appid) = resolved_appid {
        ...
        if !urls.is_empty() {
            ...
            state
                .connections
                .forward_webhook(&urls, fwd_appid, &data, timeout, fwd_stats)
                .await;
        }
    }
    ...
    state
        .connections
        .send_to_all(secret, &data, &payload, &state.stats, &state.cache)
        .await;
